...
if (
    ...
    ($_SESSION['role'] != 'SACHIV' && $_SESSION['role'] != 'CEO' && $_SESSION['role'] != 'HM')
){
...
<div id="wrapper">
    ...
    <div class="card p-5 text-center shadow-lg border-0 text-white" style="border-radius: 16px; background: linear-gradient(135deg, #6a1b9a 0%, #8e44ad 35%, #b76db8 65%, #f5c542 100%);">
        ...
        <p class="fs-5 mx-auto" style="max-width: 600px; color: rgba(255,255,255,0.95);">
            The <strong>Utility Master</strong> module is currently under active development. Our team is building tools to manage system utilities and configurations.
        </p>
        ...
        <div class="mt-4">
            <?php
            $back_url = 'sachiv_dashboard.php';
            if ($role === 'CEO') {
                $back_url = 'ceo_dashboard.php';
            } else if ($role === 'HM') {
                $back_url = 'hm_dashboard.php';
            }
            ?>
            <a href="<?php echo $back_url; ?>" class="btn btn-light px-4 py-2 fw-bold text-dark shadow-sm" style="border-radius: 8px;">
